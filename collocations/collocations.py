...
class CollocationClass:
		...
		while len(liste)>=min:
			...
			for i in range(min):
				...
				# else:
					...
					# break;
			if sublist:
				# print "bsublis",len(sublist),  u' '.join(sublist).encode('utf8');
	...
	def lookupForLongCollocations(self,inputtext):
		"""
		...
		for g in collocation_const.GENERAL_COLLOCATIONS.keys():
			#~ inputtext = inputtext.replace(g,collocation_const.GENERAL_COLLOCATIONS[g].get('v','') );
