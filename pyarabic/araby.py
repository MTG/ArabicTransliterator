...
def order(archar):
    ...
    @rtype: integer;
    """
    return AlphabeticOrder.get(archar, 0);
...
def shaddalike(partial,fully):
    ...
    if not hasShadda(partial):
        return True;
    #المدخل به شدة، والنتيجة ليس بها شدة، خاطئ
    elif not hasShadda(fully) and hasShadda(partial):
        return False;
