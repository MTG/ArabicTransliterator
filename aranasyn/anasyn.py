...
class SyntaxAnalyzer:
    ...
    def analyze(self, detailed_stemming_dict):
        ...
        for stemmingList in detailed_stemming_dict:
            tmpList = [aranasyn.stemmedsynword.stemmedSynWord(stemmingList[order], order) for order in range(len(stemmingList))]
            if not tmpList:
                tmpList = [aranasyn.stemmedsynword.stemmedSynWord(stemmingList[0]),]
            stemmedSynWordListList.append(tmpList)
            #create the synode object from tmplist
            synNodeList.append(aranasyn.synnode.synNode(tmpList))
        #----
        #print synNodeList
        ...
        if not previous or previous.isInitial():
            if current.isMarfou3() or current.isPast() or current.isStopWord():
             # if  current.isMajrour():
                ...
                current.addPrevious(aranasyn.syn_const.PrimateRelation)
            ...
            # add a relation to previous
                previous.addNext(aranasyn.syn_const.TanwinRelation)
            return (previous, current)
        elif current.isPounct() or current.isStopWord() or  (current.hasProcletic()  \
                  and not current.isDefined()):
        ...
        if previous.isStopWord():
            ...
            if current.isNoun() or current.isAddition():
                ...
                if current.isMajrour():
                    if previous.isAdded():
                        weight = aranasyn.syn_const.JarMajrourRelation
                    elif previous.isNoun() and not previous.isDefined() \
                           and not previous.isAdded() and not previous.isTanwin() :
                        weight = aranasyn.syn_const.JarMajrourRelation
                # منعوت والنعت
                #  تحتاج إلى إعادة نظر
    ...
    def isRelated(self, previous, current):
        ...
        @rtype: boolean
        """
        if (previous and current) and previous.getOrder() in current.getPrevious() \
                 and current.getOrder() in previous.getNext():
            return True
        else: return False
    ...
    def areCompatible(self, previous, current):
        ...
        if (current.isDefined() and previous.isDefined()) \
             or  (not current.isDefined() and not previous.isDefined()):
            compatible = True
        else:
            return False
        # الحركة
        # تساوي الحالة الإعرابية
        if (current.isMajrour() and previous.isMajrour()) \
            or (current.isMansoub() and previous.isMansoub()) \
            or (current.isMarfou3()and previous.isMarfou3()):
            compatible = True
        else:
            return False
        ...
        if not current.hasProcletic() or current.getProcletic() in (u"ال", u"فال" , u"وال", u"و", u"ف"):
            ...
            return False
        # التنوين
        if (current.isTanwin() and previous.isTanwin()) \
             or (not current.isTanwin() and not previous.isTanwin()):
            compatible = True
        else:
            return False
        ...
        #مؤنث ومؤنث
        # جمع التكسير مؤنث
        if (current.isFeminin() and previous.isFeminin()) \
           or (current.isMasculin() and previous.isMasculin())  \
           or (previous.isBrokenPlural() and current.isFeminin()):
            compatible = True
        else: return False
