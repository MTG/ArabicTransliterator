...
#			noun_list.append(HAMZA+ALEF+noun[1:])
		for noun in noun_list:
			...
			for seg in list_seg_comp:
				...
				for vocalized_encletic in stem_noun_const.COMP_SUFFIX_LIST_TAGS[encletic]['vocalized']:
					seg_comp_voc={'procletic':procletic,'encletic':vocalized_encletic,'stem':stem}
					# verify compatibility between procletics and afix
					list_seg_comp_voc.append(seg_comp_voc)
			list_seg_comp=list_seg_comp_voc;
			
			
			
			for seg_comp in list_seg_comp:
				# procletic=noun[:seg[0]];
				# stem=noun[seg[0]:seg[1]]
				# encletic=noun[seg[1]:]
				procletic = seg_comp['procletic'];
				stem	  = seg_comp['stem']
				encletic  = seg_comp['encletic']	
				secondsuffix=u'';
				...
				if encletic!="":
					...
					if stem.endswith(araby.YEH):
						...
						list_stem.append(stem+araby.NOON);
					# elif stem.endswith(araby.WAW_HAMZA):
						# list_stem.append(stem+araby.HAMZA);
					# elif stem.endswith(araby.YEH_HAMZA):
						# list_stem.append(stem+araby.HAMZA);							
...
##							print ' *****','-'.join(listsingle).encode('utf8')
	...
	def validateTags(self, noun_tuple, affix_tags,procletic, encletic, suffix):
		"""
...
#تستعمل في حالة كسر هاء الضمير في الجر			
		if u"مجرور" in encletic_tags and u"مجرور" not in suffix_tags:
			return False;			
		return True
	...
	def vocalize(self,noun, proclitic,prefix,suffix,enclitic):
		...
		if (proclitic.endswith(araby.ALEF+araby.LAM) or proclitic.endswith(araby.LAM+araby.LAM)) and araby.isSun(noun[0]):
