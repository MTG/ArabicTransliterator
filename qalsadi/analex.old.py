...
class analex :
	...
	def __del__(self):
		...
		"""
		#~ if self.allowCacheUse:		
			#~ # print 'cache checked word', len(self.cache['checkedWords']);
			#~ # print 'cache freqwords noun', len(self.cache['FreqWords']['noun']);
			#~ # print 'cache freqwords verb', len(self.cache['FreqWords']['verb']);
			#~ # print 'cache freqwords stopword', len(self.cache['FreqWords']['stopword']);
			#~ del(self.cache);
		self.wordfreq=None;
...
The analyzed data given by morphological analyzer Qalsadi have the following format:
		...
		if self.allowDisambiguation :		
			...
			# print '################', len(newWordlist), len(list_word)
		...
		if mode=='all':
			for i in range(len(list_word[:self.limit])):
				...
				if self.allowCacheUse and self.cache.isAlreadyChecked(word):					
					resulted_data.append(self.cache.getChecked(word));
				else:
					...
					if self.allowCacheUse: self.cache.addChecked(word, one_data_list);
	...
	def check_word(self,word, guessedTag=""):
		...
		if len(resulted_data)==0:
			resulted_data.append(stemmedword.stemmedWord({
			'word':word,          
			'procletic':'',
			'encletic':'',
			'prefix':'',
	...
	def check_shadda(self, word_vocalised,resulted_data):
		...
		"""
		#print word_vocalised.encode('utf8');
		filtred_data = filter(lambda item: araby.shaddalike(word_vocalised, item.__dict__.get('vocalized','')), resulted_data)
		# filtred_data=[];
		# for item in  resulted_data:
	...
	def check_word_as_stopword(self,word):
		...
		"""	
		detailed_result=[]
		detailed_result = self.stopwordsstemmer.stemming_stopword(word);
		# add frequency 
