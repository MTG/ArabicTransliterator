...
verbPattern[100]= re.compile(r"[%s%s%s%s%s]"%(araby.ALEF_HAMZA_BELOW, araby.TEH_MARBUTA, araby.FATHATAN, araby.DAMMATAN, araby.KASRATAN),re.UNICODE)
verbPattern[121] =re.compile(r"[%s](.)+"% araby.ALEF_MAKSURA, re.UNICODE)
# the word ends with wa  a is  araby.WAW  araby.ALEF , is a verb
verbPattern[160] = re.compile(r"([^%s%s%s]..)%s%s$"%( araby.ALEF_HAMZA_ABOVE,  araby.WAW ,  araby.FEH,  araby.WAW ,  araby.ALEF), re.UNICODE) 
# the word is started by  araby.NOON , before REH or araby.LAM, or  araby.NOON , is a verb and not a noun
verbPattern[10] =re.compile(r"^%s[%s%s%s]"%( araby.NOON , araby.REH, araby.LAM, araby.NOON ),re.UNICODE)
# the word is started by  araby.YEH,
# before some letters is a verb and not a noun
verbPattern[20] = re.compile(r"^%s[%s%s%s%s%s%s%s%s%s%s%s%s%s]"%( araby.YEH, araby.THAL, araby.JEEM, araby.HAH, araby.KHAH, araby.ZAIN, araby.SHEEN, araby.SAD, araby.DAD, araby.TAH, araby.ZAH, araby.GHAIN, araby.KAF, araby.YEH), re.UNICODE)
...
verbPattern[30] = re.compile(r"[%s%s%s%s%s]\*%s\*\*"%(araby.ALEF, araby.YEH, araby.NOON , araby.TEH ,araby.ALEF_HAMZA_ABOVE, araby.TEH ), re.UNICODE)
# the word is like ift3l pattern
verbPattern[40] = re.compile(r"[%s%s%s%s%s]%s\*\*\*"%(araby.ALEF, araby.YEH, araby.NOON , araby.TEH ,araby.ALEF_HAMZA_ABOVE, araby.NOON ), re.UNICODE)
# the word is like isf3l pattern
verbPattern[50] = re.compile(r"[%s%s%s%s%s]%s%s([^%s%s%s]{2})([^%s%s%s%s])"%(araby.ALEF, araby.YEH, araby.NOON , araby.TEH ,araby.ALEF_HAMZA_ABOVE, araby.SEEN , araby.TEH ,araby.ALEF, araby.YEH, araby.WAW ,araby.ALEF,araby.HEH,araby.KAF, araby.NOON ),re.UNICODE)
# the word contains y|t|A)st*
# يست، أست، نست، تست
verbPattern[60] = re.compile(r"(%s|%s|%s|%s)%s%s\*"%(araby.ALEF_HAMZA_ABOVE, araby.YEH, araby.TEH , araby.NOON , araby.SEEN , araby.TEH ), re.UNICODE) 
# the word contains ist***
# استفعل
verbPattern[70]= re.compile(r"%s%s%s\*\*\*"%(araby.ALEF, araby.SEEN , araby.TEH ), re.UNICODE) 

# the word contains ***t when **+t+* t is  araby.TEH 
# if  araby.TEH  is followed by  araby.MEEM , araby.ALEF,  araby.NOON 
# تم، تما، تن، تا، تني
# حالة تنا غير مدرجة
verbPattern[80] = re.compile(r"\*\*\*%s(%s|%s|%s[^%s])"%( araby.TEH , araby.MEEM ,araby.ALEF, araby.NOON ,araby.ALEF), re.UNICODE) 
#To reDo
...
##    	if re.compile(r"^([^\*])*%s(\*\*\*\*)"%(araby.ALEF), re.UNICODE) :
##
##    	    return -150;
...
nounPattern[1000]=re.compile(r"^[%s%s%s%s%s%s]"%( araby.WAW_HAMZA, araby.YEH_HAMZA, araby.FATHA,araby.DAMMA,araby.SUKUN,araby.KASRA),re.UNICODE)

# HAMZA BELOW araby.ALEF
nounPattern[1010]=re.compile(r"[%s%s%s%s%s]"%(araby.ALEF_HAMZA_BELOW, araby.TEH_MARBUTA,araby.FATHATAN,araby.DAMMATAN,araby.KASRATAN),re.UNICODE)

nounPattern[1020]=re.compile(r"[%s](.)+"%araby.ALEF_MAKSURA,re.UNICODE)

# the word is like ift3al pattern
nounPattern[1030]=re.compile(r"%s([^%s%s%s%s%s]%s|[%s%s%s]%s|[%s]%s)(.)%s([^%s%s%s])"%(araby.ALEF,araby.LAM, araby.SEEN ,araby.SAD,araby.DAD,araby.ZAH, araby.TEH ,araby.SAD,araby.DAD,araby.ZAH,araby.TAH,araby.ZAIN,araby.DAL,araby.ALEF,araby.HEH,araby.KAF, araby.NOON ), re.UNICODE)

# the word is like inf3al pattern
nounPattern[1040]=re.compile(r"%s%s(..)%s([^%s%s%s])"%(araby.ALEF, araby.NOON ,araby.ALEF,araby.HEH,araby.KAF, araby.NOON ), re.UNICODE)

# the word is like isf3al pattern
nounPattern[1050]=re.compile(r"%s%s%s(..)%s([^%s%s%s])"%(araby.ALEF, araby.SEEN , araby.TEH ,araby.ALEF,araby.HEH,araby.KAF, araby.NOON ), re.UNICODE)

# the word is finished by HAMZA preceded by araby.ALEF
#and more than 2 originals letters
nounPattern[1060]=re.compile(r"[^%s%s%s%s%s%s%s%s]{2,}%s%s(.)*"%( araby.YEH, araby.FEH,araby.LAM, araby.NOON ,araby.ALEF_HAMZA_ABOVE, araby.TEH , araby.WAW ,araby.ALEF,araby.ALEF, araby.HAMZA), re.UNICODE)

# the word contains three araby.ALEF,
# the kast araby.ALEF musn't be at end
nounPattern[1070]=re.compile(r"^(.)*([%s](.)+){3}$"%(araby.ALEF), re.UNICODE)

# the word is started by beh, before BEH,FEH, araby.MEEM 
#is a noun and not a verb
nounPattern[1080]=re.compile(r"^%s[%s%s%s]"%(araby.BEH,araby.BEH,araby.FEH, araby.MEEM ), re.UNICODE)

# the word is started by  araby.MEEM , before BEH,FEH, araby.MEEM 
#is a noun and not a verb
nounPattern[1090]=re.compile(r"^%s[%s%s%s]"%( araby.MEEM ,araby.BEH,araby.FEH, araby.MEEM ), re.UNICODE)

# the word is started  by araby.ALEF araby.LAM
# and the  original letters are more than two,
nounPattern[1120]=re.compile(r"^[%s|%s]?[%s|%s]?%s%s(.){3,8}"%( araby.FEH, araby.WAW ,araby.KAF,araby.BEH,araby.ALEF,araby.LAM), re.UNICODE)
nounPattern[1121]=re.compile(r"^[%s|%s]?%s%s(.){3,8}"%(araby.FEH, araby.WAW ,araby.LAM,araby.LAM), re.UNICODE)
...
nounPattern[1160]=re.compile(r"\*\*%s\*"%( araby.YEH),re.UNICODE) 

# the word contains al*Y* when araby.ALEF-araby.LAM+*+ araby.YEH+*is  araby.YEH
nounPattern[1170]=re.compile(r"%s%s\*%s\*"%(araby.ALEF,araby.LAM, araby.YEH),re.UNICODE) 

# the word contains al*w* when araby.ALEF-araby.LAM+*+ araby.WAW +*  w is  araby.WAW 
nounPattern[1180]=re.compile(r"%s%s\*%s\*"%(araby.ALEF,araby.LAM, araby.WAW ),re.UNICODE) 

# the word contains ***w* when ***+ araby.WAW +* w is  araby.WAW 
nounPattern[1190]=re.compile(r"[^%s]\*\*%s\*"%(u"تاينلفأو", araby.WAW ),re.UNICODE) 

# the word contains **a* when **+a+* a is araby.ALEF
nounPattern[1200]=re.compile(r"\*[\*%s]%s\*"%(u"وي",araby.ALEF),re.UNICODE) 

# the word contains t**y* when **+t+* a is araby.ALEF
nounPattern[1210]=re.compile(r"%s\*\*%s\*"%( araby.TEH , araby.YEH),re.UNICODE) 
...
nounPattern[1100]=re.compile(r"^[%s|%s]?[%s|%s]?((.){2,7})(%s|%s|%s)%s$"%(araby.FEH, araby.WAW , araby.SEEN ,araby.LAM, araby.WAW , araby.YEH,araby.ALEF, araby.NOON ), re.UNICODE)
nounPattern[1101]=re.compile(r"^[%s|%s]?[%s|%s]?[%s%s%s%s%s%s]"%(araby.FEH, araby.WAW , araby.SEEN ,araby.LAM, araby.YEH, araby.TEH ,araby.ALEF_HAMZA_ABOVE,araby.ALEF_MADDA, araby.NOON ,araby.ALEF), re.UNICODE)
